...
const aktualisierenButton = document.getElementById('aktualisieren');
    ...
    function handlePdfExport() {
        // Hier dein PDF-Export-Code mit jsPDF
        alert('PDF Export gedrückt!'); // Zum Testen
    }

      function handleExcelExport() {
        // Hier dein Excel-Export-Code mit XLSX
        alert('Excel Export gedrückt!'); // Zum Testen
    }

        function aktualisierenFunktion() {
        // Hier deine Aktualisierungslogik (z.B. Daten neu laden)
        alert('Aktualisieren gedrückt!'); // Zum Testen
    }

    // Die eigentlichen Listener:
    if (downloadPdfButton) downloadPdfButton.addEventListener('click', handlePdfExport);
    if (downloadExcelButton) downloadExcelButton.addEventListener('click', handleExcelExport);
    if (aktualisierenButton) aktualisierenButton.addEventListener('click', aktualisierenFunktion);
    ...
    let mietstationenList = []; // Array to store stations for easy iteration
    ...
    function calculateKPIs() {
      if (!vermietungenData.length || !fahrzeugeData || !mietstationenData || !kostenData) {
        ...
        const rentalCountsByStation = {}; // For chart data
        const revenueByStation = {};      // For chart data
        const revenueByMonth = {};        // For chart data (2025 specific)
        const revenueByVehicleType = {};  // For chart data
        const damageReportedCounts = { true: 0, false: 0 }; // For chart data


        // Initialize station counts and revenues
